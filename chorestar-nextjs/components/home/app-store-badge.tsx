import Link from 'next/link'
import { ACCENT_SURFACE_STYLE } from '@/lib/constants/brand'

export const APP_STORE_URL = 'https://apps.apple.com/app/id6761279049'
/** Opens the App Store's "Write a Review" sheet directly (on Apple devices). */
export const APP_STORE_REVIEW_URL = 'https://apps.apple.com/app/id6761279049?action=write-review'

/** Where someone asks for a spot in the Android test. */
export const ANDROID_BETA_URL = '/android-beta'

/**
 * The Play listing. It 404s for the public until the app is published to
 * production or open testing, so nothing links to it yet; swap it into
 * AndroidBetaBadge on launch day.
 */
export const PLAY_STORE_URL = 'https://play.google.com/store/apps/details?id=com.chorestar.family'

const BASE =
  'inline-flex items-center gap-2.5 px-5 py-2.5 rounded-full text-sm font-semibold ' +
  'transition-transform hover:scale-[1.03] focus-visible:outline-none focus-visible:ring-2 ' +
  'focus-visible:ring-offset-2 focus-visible:ring-indigo-400 dark:focus-visible:ring-offset-gray-900'

/**
 * "Download on the App Store". Drawn in-house rather than hotlinking Apple's
 * asset through our CSP, and filled with the live accent instead of Apple's
 * black slab, which sat on our cards like a foreign object. ACCENT_SURFACE_STYLE
 * carries the ink that reads on whatever accent is active.
 */
export function AppStoreBadge({ className = '', label = 'Download on the App Store' }: { className?: string; label?: string }) {
  return (
    <Link
      href={APP_STORE_URL}
      target="_blank"
      rel="noopener noreferrer"
      aria-label="Download ChoreStar on the App Store"
      className={`${BASE} shadow-sm ${className}`}
      style={ACCENT_SURFACE_STYLE}
    >
      <svg viewBox="0 0 384 512" className="w-5 h-5 fill-current shrink-0" aria-hidden="true">
        <path d="M318.7 268.7c-.2-36.7 16.4-64.4 50-84.8-18.8-26.9-47.2-41.7-84.7-44.6-35.5-2.8-74.3 20.7-88.5 20.7-15 0-49.4-19.7-76.4-19.7C63.3 141.2 4 184.8 4 273.5q0 39.3 14.4 81.2c12.8 36.7 59 126.7 107.2 125.2 25.2-.6 43-17.9 75.8-17.9 31.8 0 48.3 17.9 76.4 17.9 48.6-.7 90.4-82.5 102.6-119.3-65.2-30.7-61.7-90-61.7-91.9zm-56.6-164.2c27.3-32.4 24.8-61.9 24-72.5-24.1 1.4-52 16.4-67.9 34.9-17.5 19.8-27.8 44.3-25.6 71.9 26.1 2 49.9-11.4 69.5-34.3z" />
      </svg>
      {label}
    </Link>
  )
}

/**
 * The Android counterpart. Outlined rather than filled so the live iOS app
 * stays the primary action while Android is still a test.
 */
export function AndroidBetaBadge({ className = '', label = 'Join the Android beta' }: { className?: string; label?: string }) {
  return (
    <Link
      href={ANDROID_BETA_URL}
      aria-label="Join the ChoreStar Android beta"
      className={`${BASE} border-2 border-indigo-200 dark:border-indigo-800 text-indigo-700 dark:text-indigo-300 bg-white dark:bg-gray-800 hover:border-indigo-300 dark:hover:border-indigo-700 ${className}`}
    >
      <svg viewBox="0 0 512 512" className="w-5 h-5 fill-current shrink-0" aria-hidden="true">
        <path d="M325.3 234.3 104.6 13l280.8 161.2-60.1 60.1zM47 0C34 6.8 25.3 19.2 25.3 35.3v441.3c0 16.1 8.7 28.5 21.7 35.3l256.6-256L47 0zm425.2 225.6-58.9-34.1-65.7 64.5 65.7 64.5 60.1-34.1c18-14.3 18-46.5-1.2-60.8zM104.6 499l280.8-161.2-60.1-60.1L104.6 499z" />
      </svg>
      {label}
    </Link>
  )
}
