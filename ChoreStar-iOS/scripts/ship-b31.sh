#!/bin/bash
# ChoreStar 2.2 (build 31): archive -> re-stamp -> upload -> notes in four
...
#
# 2.2 = the redesign release: family-first hero (per-kid avatar rings, tap
# to jump to a kid), the restraint pass (solid accent, quiet cards, Bricolage
# Grotesque display face, factual copy), and the Home Family section retired.
# The refreshed App Store screenshots are ALREADY on the 2.2 listing
# (uploaded Sep 7 via fastlane metadata), so this lane only handles the
# binary, notes, and submission.
#
